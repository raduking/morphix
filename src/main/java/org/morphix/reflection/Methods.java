...
public interface Methods {
	...
	/**
	 * Returns a method in the class given as parameter, also searching in all
	 * it's super classes.
	 *
	 ...
	 * </ul>
	 * The returned order of the methods are: class -> super class -> ... -> base class
	 * and all methods in each class are returned in the declared order.
	 *
	 * @param cls class on which the fields are returned
	...
	/**
	 * Returns a list with all the methods in the class given as parameter
	 * including the ones in all it's super classes which verify the given
	 * method predicate.
	 *
	 * @param cls class on which the fields are returned
	 * @return list of fields
	...
	/**
	 * Invokes the given method on the given object with parameters, ignoring
	...
	static <T, R> R invokeIgnoreAccess(final Method method, final T obj, final Object... args) {
	...
	/**
	 * Invokes the given method on the given object with parameters, throws just
	 * the original exception message when needed.
	 *
	 * @param <T> object type on which the method is invoked
	 * @param <R> method return type
	 *
	 ...
	 * </pre>
	 *
	 * <code>getGenericReturnType(fooMethod, 0)</code> will return
	 * <code>String.class</code>.
	 * <p>
	 *
	 * @param method method for which the generic return type is needed
	 * @param index the zero based index of the type needed (for a Map the 2nd
	 *            generic parameter has index 1)
	...
	/**
	 * Returns the generic return type for a method or null if method has no
	 * generic return type.
	 *
	 * @param method method for which the generic return type is needed
	 * @param index the zero based index of the type needed (for a Map the 2nd
	 ...
	 * </pre>
	 *
	 * <code>getGenericReturnClass(fooMethod, 0)</code> will return
	 * <code>String.class</code>.
	 * <p>
	 *
	 * @param method method for which the generic return type is needed
	...
	/**
	 * Invokes all methods that are annotated with the given annotation.
	 * The annotated method should have no parameters and should return <code>void</code>
	 *
	...
	static <T, A extends Annotation> void invokeMethodsWithAnnotation(final T obj, final Class<A> annotationClass) {
		...
		for (Method method : methods) {
			Methods.invokeIgnoreAccess(method, obj);
		}
	}
	...
	/**
	 * Returns the currently executing method name. The advantage of this method is
	 * that each {@link StackTraceElement} is fetched lazily, so you don't actually
	 * construct the full stack trace before checking the first method.
	 *
	 ...
	 */
	static <T> Optional<String> getCallerMethodName(final Supplier<T> supplier, final BiFunction<? super String, ? super String, String> nameFunction) {
