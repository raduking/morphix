...
public class Enums {
	 ...
	 * example {@link Enum#name()}, {@link Enum#toString()}, etc. If there are duplicate keys generated by the mapper
	 * function, the first one is kept.
	 *
	 * @param <T> enum type
	...
	public static <K, T extends Enum<T>> Map<K, T> buildNameMap(final T[] values, final Function<T, K> mapper) {
		return Stream.of(values).collect(Collectors.toUnmodifiableMap(mapper, Function.identity(), BinaryOperators.first()));
	}
	 ...
	 * @param nameMap type to enum value map
	 * @param values expected values for an enum
	 * @return enum value
	 */
	public static <K, T extends Enum<T>> T from(final K value, final Map<K, T> nameMap, final T[] values) {
		...
		if (null == enumValue) {
			throw new IllegalArgumentException("'" + value + "' has no corresponding value. Accepted values: " + List.of(values));
		}
		return enumValue;
	}
